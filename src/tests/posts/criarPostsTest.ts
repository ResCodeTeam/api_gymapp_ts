    ...
    it('Deve retornar post criado', () => {
      ...
      .then(res => {
        ...
        if(res.body['imagens_publicacao'].length>0){
          ...
          chai.expect(res.body['imagens_publicacao'][0]).to.have.property("url")

          // verificar se url da imagem é uma string
          chai.expect(res.body['imagens_publicacao'][0]['url']).to.be.a("string")
        ...
        if(res.body['identificacoes_publicacoes'].length>0){
          //verificar se objeto do array é um objeto
          chai.expect(res.body['identificacoes_publicacoes'][0]).to.be.an("object")
          // verificar se existe users do gosto
          ...
          if(res.body['identificacoes_publicacoes'][0]['users']['imagem_url']!=null)
